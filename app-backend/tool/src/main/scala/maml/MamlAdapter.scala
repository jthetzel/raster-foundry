  ...
  def asMaml: (Expression, Option[NodeMetadata]) = {
    def eval(ast: MapAlgebraAST): Expression = {
      ...
      ast match {
        ...
        case MapAlgebraAST.Classification(_, _, _, classmap) => Classification(args, MamlClassMap(classmap.classifications))
        case MapAlgebraAST.Masking(_, _, _, mask) => Masking(args :+ GeomJson(mask.toGeoJson))
        ...
        /* --- FOCAL OPERATIONS --- */
        case MapAlgebraAST.FocalMax(_, _, _, n) => FocalMax(args, NeighborhoodConversion(n))
        case MapAlgebraAST.FocalMin(_, _, _, n) => FocalMin(args, NeighborhoodConversion(n))
        case MapAlgebraAST.FocalMean(_, _, _, n) => FocalMean(args, NeighborhoodConversion(n))
        case MapAlgebraAST.FocalMedian(_, _, _, n) => FocalMedian(args, NeighborhoodConversion(n))
        case MapAlgebraAST.FocalMode(_, _, _, n) => FocalMode(args, NeighborhoodConversion(n))
        case MapAlgebraAST.FocalSum(_, _, _, n) => FocalSum(args, NeighborhoodConversion(n))
        case MapAlgebraAST.FocalStdDev(_, _, _, n) => FocalStdDev(args, NeighborhoodConversion(n))
      }
    }
