...
object NDWI extends (MultibandTile => Tile) {
  val colorRamp = ColorRamp(Array[Int](0xaacdff44, 0x70abffff, 0x3086ffff, 0x1269e2ff, 0x094aa5ff, 0x012c69ff))
  ...
  def apply(tile: MultibandTile): Tile = {
    tile.convert(DoubleCellType).combineDouble(0, 1) { (g, nir) => (g - nir) / (g + nir) }
  }
}
